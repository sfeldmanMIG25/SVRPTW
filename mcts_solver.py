...
class MCTSAgent:
    ...
    def run_mcts(self, root_state, vehicle_idx):
        ...
        possible_actions = self.engine.get_feasible_actions(vehicle_idx, root_state)
        ...
        for _ in range(MCTS_ITERATIONS):
            ...
            if node.untried_actions:
                ...
                child_node.untried_actions = self.engine.get_feasible_actions(current_v_idx, next_state)
            ...
            for _ in range(ROLLOUT_DEPTH):
                ...
                acts = self.engine.get_feasible_actions(current_v_idx, temp_state)
                if not acts: break
...
def run_mcts_pipeline():
    ...
    print(f"--- Starting MCTS Evaluation on {len(files)} Instances ---")
