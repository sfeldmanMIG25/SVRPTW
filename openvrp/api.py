...
def solve(problem: Problem, options: SolveOptions | None = None) -> Solution:
    ...
    # Populate the operator_pool BEFORE any phase fires so even an
    # ingest-phase StopSolve returns a Solution with the pool surfaced.
    diag.operator_pool = active_operator_pool(problem)

    # Top-level StopSolve guard. The spec contract is "StopSolve raised
    # from on_progress ends the search cleanly with best-so-far"; the
    # inner pipeline catches StopSolve at every phase boundary and
    # routes through finalize when possible. Anything that escapes the
    # inner is a genuine cancellation before construction began.
    try:
        return _solve_inner(problem, options, t0, deadline, diag)
    except StopSolve:
        return _empty_solution_with_failure(
            problem, "solve cancelled by on_progress (StopSolve) at ingest",
            options, diag, wall=time.monotonic() - t0,
            status="partial")


def _solve_inner(problem: Problem, options: SolveOptions, t0: float,
                 deadline: float, diag: SolveDiagnostics) -> Solution:
    """The actual solve pipeline. Wrapped by ``solve()`` with a
    top-level StopSolve guard."""
    # ---- 1. INGEST ----
    ...
    # operator_pool already populated in solve() before the StopSolve guard.
    ...
    try:
        sv_sol = _dispatch_solver(inst, settings, pool_construction,
                                  ...
                                  construction_variant=options.construction_variant,
                                  blink_prob=options.blink_prob)
    except StopSolve:
        # Cancelled before construction returned anything usable.
        return _empty_solution_with_failure(
            problem, "solve aborted by user during construct/refine",
            options, diag, wall=time.monotonic() - t0, status="partial")
    except Exception as e:
        return _empty_solution_with_failure(problem, f"solver raised: {e!r}", options, diag,
                                            wall=time.monotonic() - t0)

    # The refine progress event is the LAST place the user can intercept
    # before finalize. If they cancel here, finalize the constructed
    # sv_sol as best-so-far — don't discard the work the solver just did.
    refine_cancelled = False
    try:
        _fire(options, ProgressEvent(
            fraction=0.85,
            elapsed_seconds=time.monotonic() - t0,
        ...
        refine_cancelled = True

    t_construct_done = time.monotonic()

    # ---- 3. FINALIZE (always runs; ensures every Solution has a
    #         populated quality_report, fleet_min_report, etc.) ----
    ...
    if refine_cancelled:
        # Mark the cancellation in diagnostics — the Solution itself is
        # still valid (best-so-far); the caller asked us to stop.
        diag.feasibility_blockers = list(diag.feasibility_blockers) + [
            "solve cancelled by on_progress (StopSolve) at refine; "
...
def _dispatch_solver(inst: Any, settings: Any, construction: str,
                     remaining: float, seed: int,
                     *, construction_variant: str = "nn",
                     blink_prob: float = 0.0) -> Any:
...
def _empty_solution_with_failure(problem: Problem, message: str,
                                 ...
                                 *, wall: float,
                                 status: str = "infeasible") -> Solution:
    """Return an empty Solution carrying the failure / cancellation reason.

    ``status='infeasible'`` is the default (true failure); pass
    ``status='partial'`` for clean cancellation where the caller asked
    the search to stop before a Solution was constructed.
    """
    from openvrp.schema.output import QualityReport, RouteEconomics
    diag.feasibility_blockers = [message]
    # Always populate the fleet_min_report and quality_report — the
    # safety contract is that EVERY returned Solution has these
    # diagnostics filled, even on early termination.
    if not diag.fleet_min_report:
        diag.fleet_min_report = {
            "used": 0.0, "minimum_found": 0.0,
            "objective_delta_if_minimized": 0.0,
        }
    from openvrp.schema.input import QUALITY_CATALOG
    empty_quality = {k: 0.0 for k in QUALITY_CATALOG}
    return Solution(
        ...
        quality_report=QualityReport(solution_level=empty_quality, per_route=[]),
        vehicles_used=0,
