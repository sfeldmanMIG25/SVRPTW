...
class Depot(BaseModel):
    ...
    **Generalized resource replenishment** (multi-depot, multi-resource):
    ``resources`` declares which resource types this depot stocks and their
    per-depot capacity. A value of ``None`` means unlimited stock; a float
    means a hard daily/session cap; a missing key means the depot does NOT
    stock that resource. Vehicles whose ``VehicleClass.consumes`` requires
    a resource can replenish only at depots that stock it.
    """
    ...
    resources: dict[str, float | None] = Field(default_factory=dict,
        description="Per-resource stock at this depot. Key = resource name "
                    "(e.g. 'fuel_liters', 'spare_parts'); value = capacity "
                    "(None = unlimited; missing key = not stocked). "
                    "Default empty = no replenishment offered (back-compat).")
...
class VehicleClass(BaseModel):
    ...
    consumes: dict[str, float] = Field(default_factory=dict,
        description="Per-resource consumption per meter of route distance. "
                    "Key = resource name (must match a Depot.resources key for "
                    "any depot at which this class can replenish); value = "
                    "units consumed per meter. Default empty = no replenishment "
                    "needed (back-compat with non-resource workloads).")
    onboard_capacity: dict[str, float] = Field(default_factory=dict,
        description="Per-resource tank/storage capacity carried by one vehicle "
                    "of this class. The vehicle starts a route with a full tank "
                    "and must replenish at a depot before the tank hits zero. "
                    "Resource keys should align with ``consumes``; missing keys "
                    "are treated as 'unlimited tank' for that resource.")
...
class SolveOptions(BaseModel):
    ...
    construction_variant: Literal["nn", "regret2", "regret3"] = Field(
        default="nn",
        description="Native-solver construction strategy (when construction='fast' or 'auto' "
                    "with no pyvrp). 'nn'=parallel nearest-neighbor (cheapest); "
                    "'regret2'/'regret3'=regret-k insertion (Solomon-style I1 with "
                    "regret tie-breaker; slower but higher initial quality).")
    blink_prob: float = Field(default=0.0, ge=0.0, le=1.0,
        description="SISR-style blink probability for the relocate phase. "
                    "0.0 = deterministic greedy; 0.05 ≈ SISR sweet spot. "
                    "Reference: Christiaens & Vanden Berghe, Transportation Science 2020.")
...
class Problem(BaseModel):
    ...
    def _validate_problem(self) -> Problem:
        ...
        # 7b. Generalized resource replenishment consistency.
        # If any VehicleClass.consumes references a resource, at least
        # one Depot.resources should stock it — otherwise the vehicle
        # could never replenish that resource.
        all_stocked: set[str] = set()
        for d in self.depots:
            all_stocked.update(d.resources.keys())
        for i, c in enumerate(self.fleet):
            for res_key in c.consumes:
                if res_key not in all_stocked:
                    rep.add_warning("class.resource_unstocked",
                                    f"VehicleClass {c.id!r}.consumes key "
                                    f"{res_key!r} is not stocked at any depot "
                                    f"(stocked={sorted(all_stocked) or 'none'}). "
                                    f"Vehicle cannot replenish this resource.",
                                    f"fleet[{i}=={c.id!r}].consumes")
            # If a class consumes a resource but has zero onboard_capacity for
            # it, the route can never even start (would immediately deplete).
            for res_key in c.consumes:
                cap = c.onboard_capacity.get(res_key)
                if cap is not None and cap <= 0:
                    rep.add_error("class.onboard_zero",
                                  f"VehicleClass {c.id!r}.onboard_capacity"
                                  f"[{res_key!r}]={cap} but consumes>0; "
                                  f"vehicle would deplete immediately.",
                                  f"fleet[{i}=={c.id!r}].onboard_capacity")
