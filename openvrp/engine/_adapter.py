...
def to_svrptw_instance(problem: Problem, *,
    ...
    # svrptw caches per-instance state in module-level dicts keyed by
    # ``instance_id`` (see svrptw.solvers.common.local_search._cust_by_id).
    # If two openvrp solves reuse the same id, the smaller instance's
    # customer dict can be served to a larger one, raising KeyError on
    # the missing matrix indices. Hash the (N, OD shape, t0) tuple so
    # every distinct problem gets a distinct cache key.
    import hashlib
    sig = f"{len(customers)}|{T.shape[0]}|{problem.t0_epoch}|{T[0, 1:5].tobytes().hex()[:16]}"
    instance_id = f"openvrp-{hashlib.sha256(sig.encode()).hexdigest()[:12]}"
    inst = Instance(
...
def from_svrptw_solution(sv_sol: Any, problem: Problem, *,
    ...
    # Build the OD matrices ONCE — they're invariant across all routes
    # and finalize steps. Pre-rewrite each route was re-converting
    # problem.od.{time_seconds,distance_meters} (Python nested lists) into
    # numpy arrays — ~1 MB / ~10 ms each at N=1000, ×K routes ⇒ >1 s
    # wasted on copies.
    T_arr: np.ndarray | None = None
    D_arr: np.ndarray | None = None
    if problem.mode == "od" and problem.od is not None:
        T_arr = np.asarray(problem.od.time_seconds, dtype=np.float64)
        D_arr = np.asarray(problem.od.distance_meters or problem.od.time_seconds,
                           dtype=np.float64)

    # Lazy OD-distance lookup for mean_detour_ratio — only the pairs that
    # appear as consecutive visits in some route.
    ...
    for ri, r in enumerate(sv_sol.routes):
        ...
        for cust_id in r.customers:
            ...
            if sid is None:
                continue
            # Travel time (seconds; openvrp stores seconds in problem.od)
            tt = float(T_arr[prev_mi, mi]) if T_arr is not None else 0.0
            clock += tt
            arrival = clock
        ...
        # Return-to-depot
        ret_tt = float(T_arr[prev_mi, 0]) if T_arr is not None else 0.0
        clock += ret_tt
        total_dur += ret_tt
        visits.append(Visit(
            stop_id=home_depot_id, kind="depot",
        ...
        if late_total > 1e-6 and _all_customer_tw_hard(visits, stop_by_id):
            feasible = False
    ...
    vehicles_used = len(routes_out)
    vehicles_min = _audit_fleet_minimum_size(routes_out, problem, stop_by_id)
...
def _all_customer_tw_hard(visits: list[Any], stop_by_id: dict[str, Any]) -> bool:
    """True iff every customer visit's stop has at least one hard time window.

    A route with positive lateness is only infeasible when every late
    customer's TW is hard (soft windows are priced, not blocking).
    """
    for v in visits:
        if v.kind != "customer":
            continue
        ...
        if s is None or not s.time_windows:
            return False
        if not any(tw.hard for tw in s.time_windows):
            return False
    return True
...
def _audit_fleet_minimum_size(routes_out: list[Any], problem: Problem,
                              stop_by_id: dict[str, Any]) -> int:
    """Real fleet-minimum audit (SPEC-OPENVRP-00 D7, SPEC-OPENVRP-08 I8).

    Greedy first-fit-decreasing bin-pack of per-route loads into the
    largest available class capacity. Returns the smallest number of
    bins (= the minimal sufficient fleet) — always at least 1.

    The caller's chosen routes are preserved; this is a *report*. If
    ``vehicles_minimum_found < vehicles_used`` the diagnostics tell the
    caller they could shrink the fleet by re-solving.
    """
    ...
    def route_load(r: Any) -> float:
        ...
        for v in r.visits:
            ...
            if s is None:
                continue
            for val in s.demand.values():
                load += float(val) if s.delivery_of is None else -float(val)
        return load

    loads_sorted = sorted((route_load(r) for r in routes_out), reverse=True)
    bins: list[float] = []
