...
class Instance:
    ...
    # iter-6a-5 multi_depot: optional list of additional depots. When None
    # (legacy default), evaluator + solvers use the single `depot` field
    # exclusively -> bit-identical to pre-multi-depot behaviour. When set,
    # each Route can specify `depot_idx` to start/end at any element of this
    # list (index 0 is the primary depot for backward compat).
    # Real-world equivalents: multi-warehouse delivery, cross-dock pickups,
    # regional satellite hubs.
...
def load_instance(json_path: str | Path) -> Instance:
    ...
    # iter-6a-5: optional multi-depot. If JSON has "depots" array, parse;
    # else leave None (legacy single-depot path).
    depots = None
    if "depots" in raw and raw["depots"] is not None:
        depots = [Depot(**d) for d in raw["depots"]]
    return Instance(
