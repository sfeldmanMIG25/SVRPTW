...
def _write_problem(inst: Instance, work: Path, max_seconds: int) -> tuple[Path, Path]:
    ...
    # iter-5w follow-up to wholesale LKH-3 wiring bug: at N=1000 the
    # previous (RUNS=5 + TIME_LIMIT=budget) recipe required 5 * budget s
    # but the Python-side subprocess.run timeout kills at budget*3+30 s,
    # so LKH-3 finished 0 runs and returned no feasible solution. Switch
    # to RUNS=1 with the full budget; at large N a single LK + patching
    # pass is more useful than scheduling restarts that never get to
    # finish. Small N benchmarks (N<=200) are unaffected because at
    # those sizes RUNS=1 still converges within budget.
    par = [
...
def _parse_tour(tour_path: Path, n_total: int) -> list[list[int]]:
    ...
    iter-5w follow-up: the prior parser required "Cost:" on every route
    line and silently dropped routes without it. LKH-3's MTSP_SOLUTION_FILE
    format puts a single "Cost = N" / "Penalty = N" header at the TOP and
    each route line is just node ids (or "Route #k: ids"). The Cost-on-line
    check made all routes get dropped for any non-trivial output, returning
    empty -> infeasibility cost. Fix: accept any line that contains >=2
    customer-range node ids (excluding the cost header lines).
    """
    ...
    for raw_line in text.splitlines():
        ...
        if not line:
            continue
        # Strip "Route #k:" prefix if present.
        line = re.sub(r"^Route\s*#?\d+\s*:\s*", "", line)
        # iter-5w parser-fix: route lines in LKH-3 MTSP output ALWAYS start
        # with the depot id "1 " (because tours are depot-to-depot loops).
        # Header lines like "OSM-Manhattan-N100-I000, Cost: 79_44079" and
        # "The tours traveled by the 30 salesmen are:" start with letters,
        # not "1 ". Requiring the "1 " prefix is the cleanest header filter
        # (the earlier alpha-prefix regex was defeated by hyphens in the
        # instance name).
        if not re.match(r"^1[\s\->]+\d", line):
            continue
        # Pull ids; tolerate "->" arrows and any whitespace. Truncate at
        # "(#N)" or "Cost:" so we don't pull the trailing summary digits.
        body = re.split(r"\s*\(#|\s*Cost\s*[:=]", line)[0]
        ids = re.findall(r"\d+", body)
        if len(ids) < 2:
            continue
        # Drop leading + trailing depot 1's so we don't index out-of-range.
        # The interior ids are the customers in order.
        while ids and ids[0] == "1":
            ids = ids[1:]
        ...
        for tok in ids:
            ...
            if v <= 1 or v > n_total:
                continue
            # Map LKH 1-based node ids to our 0-based customer ids.
            r.append(v - 1)
        if r:
            routes.append(r)
    return routes
