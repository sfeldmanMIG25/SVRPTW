...
def solve(inst: Instance, settings: Settings, budget_seconds: float | None = None,
    ...
    # iter-5w follow-up to wholesale regret_3 budget bug: construction is
    # O(N**3 * k) so at N>=500 it can run for thousands of seconds with no
    # budget check. The improvement phase below DOES budget; the construction
    # phase did not. Cap construction at 20% of total budget so improvement
    # actually runs. Any unrouted customers remaining at the deadline are
    # left for evaluate()'s missed-delivery penalty (same semantics as
    # explicit drops from the threshold).
    if budget_seconds and budget_seconds > 0:
        construct_deadline = t0 + max(1.0, float(budget_seconds) * 0.2)
    else:
        construct_deadline = float("inf")
    ...
    while unrouted:
        # iter-5w: hard deadline -- drop remaining unrouted if past construction
        # deadline. The improvement loop below still has 80% of budget to fix
        # things; without this gate construction can eat the full budget.
